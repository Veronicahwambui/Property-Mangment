...
function InvoicesParent() {
    ...
    const getInvoices = () => {
        let data = {startDate:startDate,endDate:endDate, size:size, page:page, search:status};
        requestsServiceService.getParentInvoices(data).then(res => {
                            ...
                            <div className="card">
                                                ...
                                                <form className="app-search d-none d-lg-block">
                                                    <div className="position-relative">
                                                        <input type="text" className="form-control"
                                                               placeholder="Search..." onChange={(e) => setStatus(e.target.value)}/>
                                                        <span className="bx bx-search-alt"></span>
                                                    </div>
                                                </form>
                                            </div>
                                            <div>
                                                <select className={"btn btn-primary"} name="" id="" value={size}
                                ...
                                <div className="card-body">
                                        ...
                                        <table className="table align-middle table-hover  contacts-table table-striped "
                                            ...
                                            {invoices.length > 0 && invoices?.map((invoice, index) => (
                                                <tr data-id={index} key={index}>
                                                    ...
                                                    <td>{moment(invoice.invoiceDate).format('MMMM Do YYYY')}</td>
                                                    <td>{invoice.paymentStatus === "PENDING" ? <span
                                                            className="badge-soft-danger badge">{invoice.paymentStatus}</span> :
                                                        <span
                                                            className="badge-soft-success badge">{invoice.paymentStatus}</span>}</td>
                                                    <td>
                                  ...
                                  <a className="dropdown-item" href="#"
                                        onClick={() => getOneInvoice(invoice.transactionId)}>
                                    <i className="font-size-15 mdi mdi-eye me-3 " href="# "></i>View
                                  </a>
                                            ...
                                            <tr>
                                                <th className="text-capitalize text-nowrap"
                                                    colSpan="12">{invoices && invoices.length} Invoices
                                                </th>
                                            </tr>
                                            </tfoot>
                                        </table>
